      ...
      <div class="fixed inset-0 flex">
        <div class="relative mr-16 flex w-full max-w-xs flex-1 flex-col bg-white pt-5 pb-4">
          ...
          <div class="flex flex-shrink-0 items-center px-4">
            <span class="text-2xl font-bold text-brand-800">SIPDOK</span>
          </div>
          <div class="mt-5 h-0 flex-1 overflow-y-auto">
            <nav class="space-y-1 px-2">
              <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="[isCurrentRoute(item.href) ? 'bg-brand-50 text-brand-800' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900', 'group flex items-center rounded-md px-2 py-2 text-base font-medium']">
                <component :is="item.icon" :class="[isCurrentRoute(item.href) ? 'text-brand-800' : 'text-gray-400 group-hover:text-gray-500', 'mr-4 h-6 w-6 flex-shrink-0']" aria-hidden="true" />
                {{ item.name }}
              </router-link>
            </nav>
          </div>
          <div class="flex flex-shrink-0 border-t border-gray-200 p-4">
            <div class="flex-shrink-0 w-full group block">
              <div class="flex items-center">
                <div>
                  <div class="inline-block h-9 w-9 rounded-full bg-brand-100 text-brand-800 flex items-center justify-center font-bold">
      ...
      <div class="flex flex-grow flex-col overflow-y-auto border-r border-gray-200 bg-white pt-5 pb-4 shadow-sm">
        ...
        <div class="mt-8 flex flex-1 flex-col">
          <nav class="flex-1 space-y-1 px-2 pb-4">
            <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="[isCurrentRoute(item.href) ? 'bg-brand-50 text-brand-800' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900', 'group flex items-center rounded-md px-2 py-2 text-sm font-medium']">
              <component :is="item.icon" :class="[isCurrentRoute(item.href) ? 'text-brand-800' : 'text-gray-400 group-hover:text-gray-500', 'mr-3 h-5 w-5 flex-shrink-0']" aria-hidden="true" />
              {{ item.name }}
            </router-link>
          </nav>
        </div>
